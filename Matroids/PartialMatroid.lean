...
remaining options.
* `elimGreater`: Seeks to make sure that we do not identify partial matroids with different orders
...
def elimGreater (l : List Nat) : List (List Nat) → List (List Nat)
  | [] => []
  | h1 :: t1 =>
    if h1 > l then
      elimGreater l t1
...
def augment (l : List Nat) (M : PartialMatroid) : PartialMatroid :=
  ...
  { matroid := (l :: M.matroid).sort, remainingOptions := elimGreater l (elimNearlySame l M.remainingOptions) }
